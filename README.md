...
仓库另附 `e2e-check.js`（Node 内置模块，无需安装依赖），在 compose 启动后对 `http://host.docker.internal:<CC_PUBLISH_PORT>` 跑 120 项接口级业务断言（覆盖六角色鉴权、名单、签到、改站、代刷、事件五方协同、考勤豁免、申诉、完整双确认矩阵、**Promise.all 并发 6 路确认与两个 operator 并发发布互斥**、访客门禁、停运通知，以及**晚点考勤豁免证明全链路：出证可信核验（准点车次手工事故说明被拒且零副作用、他车事件引用被拒）、事故公共晚点与个人到站迟到分责（25+15=40 分钟员工仅豁免事故部分并保留扣费/申诉、事故豁免人数不虚增）、多源取证、跨企业越权拦截、HR 按企业×班次批量确认/回写考勤、司机绩效分责、线路复盘双计数、员工端与 HR 端晚点原因一致性**）：
