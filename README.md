...
- **另一趟真实事故晚点车次（出证核验）**：昨日西线早班（driver02 刘安全）在高新大道匝道遭遇追尾事故，GPS 晚点 25 分钟、3 名华星/瑞丰员工已签到、事件已留痕，但**尚未出证**；可由调度在「晚点豁免证明 → 平台取证生成证明」对该车次出证，由华星/瑞丰 HR 分别按企业批量确认。对照地，今日准点到厂车次即使手工填写「道路事故」原因也会被平台拒绝（无证明、无回写、无通知）。
...
仓库另附 `e2e-check.js`（Node 内置模块，无需安装依赖），在 compose 启动后对 `http://host.docker.internal:<CC_PUBLISH_PORT>` 跑 113 项接口级业务断言（覆盖六角色鉴权、名单、签到、改站、代刷、事件五方协同、考勤豁免、申诉、完整双确认矩阵、**Promise.all 并发 6 路确认与两个 operator 并发发布互斥**、访客门禁、停运通知，以及**晚点考勤豁免证明全链路：出证可信核验（准点车次手工事故说明被拒且零副作用、他车事件引用被拒）、多源取证、跨企业越权拦截、HR 按企业×班次批量确认/回写考勤、司机绩效撤销扣分、线路复盘、员工端与 HR 端晚点原因一致性**）：
